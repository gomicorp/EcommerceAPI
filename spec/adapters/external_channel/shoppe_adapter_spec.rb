...
RSpec.describe ExternalChannel::ShopeeAdapter do
  before(:all) do
    @adapter = ExternalChannel::AdapterFactory.adapter('shopee')
    @query = {
      'country_code': 'vn',
  ...
  context 'products' do
    before do
      @parse_query_hash = @adapter.parse_query_hash(ExternalChannel::ShopeeAdapter::QUERY_MAPPER['products'], @query)
  ...
  context 'orders' do
    before do
      @parse_query_hash = @adapter.parse_query_hash(ExternalChannel::ShopeeAdapter::QUERY_MAPPER['orders'], @query)
      puts '# parse_query_hash'
      puts @parse_query_hash
      puts
    end
